...
def test_every_field_is_its_own_read_block():
    ...
    # spec to derive confirmed gaps from (devices/ac200l.py's module
    # docstring), so - like AC500 - every field stays its own block rather
    # than bridging over any address this device hasn't confirmed.
    device = AC200L(None)
